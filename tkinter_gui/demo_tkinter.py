...
for tasks_list in this_tasks_list:
    label = Label(tasks_list_frame,text=f"{tasks_list.name}")
    label.grid(sticky='e', column=0, row=custom_row, padx=50)

    custom_row += 1
custom_row = 0
for tasks_list in this_tasks_list:
    label = Label(tasks_list_frame,text=f"{tasks_list.date}")
    label.grid(sticky='e', column=1, row=custom_row)
